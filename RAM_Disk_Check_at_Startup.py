...
RAMDISK_SIZE = "2G" # Define the desired RAM disk size here
...
<body>
    <div class="wrapper">
        <div id="system-warning" class="system-warning"></div>
...
def init_app(system_status: dict):
    ...
    app.router.add_get('/api/system_status', handle_system_status)
...
def main():
    ...
    if not is_ramdisk:
        logging.warning("="*80)
        logging.warning("PERFORMANCE WARNING: Stream directory is not a RAM disk.")
        logging.warning(f"A warning will be displayed in the UI. To fix this, run:")
        logging.warning(f"    {system_status['ramdisk_command']}")
        logging.warning("="*80)
    else:
