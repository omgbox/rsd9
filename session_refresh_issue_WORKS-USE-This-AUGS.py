...
async def handle_restart_stream(request):
    ...
    with streams_lock:
        ...
        # Stop any old process but keep the files.
        stop_stream_process(session_id, cleanup_files=False)
        
        # Re-initialize the session entry for the new process, keeping old progress and context.
        active_streams[session_id] = {'process': None, 'last_seen': time.time(), 'process_running': False, 
                                      'progress_data': session_data.get('progress_data', {}), 'progress_task': None, 'status_message': 'Restarting...', 
                                      'stream_context': context}

    # Launch tasks again with the saved context and start info
