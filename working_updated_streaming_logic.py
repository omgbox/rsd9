...
<body>
    ...
    <script>
      document.addEventListener('DOMContentLoaded', function() {
          ...
          let adaptiveMinSegments = 4; // Default safe buffer, will be adjusted based on speed
                  ...
                  .then(c=>{
                      if(c&&c.includes(".ts")){
                          ...
                          if(n>=adaptiveMinSegments){
                              ...
                              fetch(s).then(r=>{const subUrl=r.ok?s:null;startPlayback(p, subUrl)})
          ...
          const startPollingForProgress=()=>{
              ...
              progressInterval=setInterval(() => {
                  fetch(`/progress/${sessionId}`).then(r=>r.json())
                  .then(data=>{
                      ...
                      if(speedInMBps<0.5){adaptiveMinSegments=6}
                      else if(speedInMBps<1.5){adaptiveMinSegments=5}
                      else if(speedInMBps>5){adaptiveMinSegments=2}
                      else{adaptiveMinSegments=4}
          ...
          streamForm.addEventListener('submit',function(e){
              ...
              .catch(err=>{alert(err.name==='AbortError'?'Error: Request timed out. No seeds?':'Error fetching file list: '+err.message)})
...
def stop_stream_process(session_id, cleanup_files=False):
    with streams_lock:
        ...
        # Handle cleanup for a session that doesn't exist in the dictionary
        ...
        # Gracefully terminate the stream process
        if (proc := session_data.get('process')) and proc.poll() is None:
            logging.info(f"Gracefully terminating stream process for session {session_id} (PID: {proc.pid})")
            proc.terminate()
            try:
                proc.wait(timeout=5)
                logging.info(f"Process {proc.pid} terminated gracefully.")
            except subprocess.TimeoutExpired:
                logging.warning(f"Process {proc.pid} for session {session_id} did not terminate in time. Sending kill signal.")
                proc.kill()
                proc.wait()
...
def start_stream_process(session_id, video_url):
    ...
    # Daemon thread is acceptable as its lifecycle is tied to the subprocess pipe
...
async def download_and_convert_subtitle(app, session_id, magnet_url, subtitle_index):
    logging.info(f"Attempting subtitle download for session {session_id}, index {subtitle_index}")
    ...
    try:
        async with app['http_client'].get(url, timeout=30) as response:
            if response.status == 200:
                ...
                logging.warning(f"Failed subtitle download (HTTP {response.status}) for session {session_id}")
...
async def handle_stream_post(request):
    ...
    p_url = f"{STREAM_API_URL}{quote(url)}&index={v_idx}" if is_magnet else url
    await asyncio.to_thread(start_stream_process, session_id, p_url)
    if is_magnet:
...
async def handle_stop_stream(request):
    ...
    if hard: logging.info(f"Performing hard reset for session {session_id}.")
    stop_stream_process(session_id, cleanup_files=hard)
...
async def cleanup_on_shutdown(app):
    ...
    for sid in list(active_streams.keys()):
        logging.info(f"Stopping stream for session {sid} due to shutdown.")
        stop_stream_process(sid, cleanup_files=True)
...
def main():
    if not os.path.ismount(STREAMS_BASE_DIR):
        logging.warning(f"PERFORMANCE WARNING: Mount a RAM disk at {os.path.abspath(STREAMS_BASE_DIR)} for best results.")
    try:
        os.makedirs(STREAMS_BASE_DIR, exist_ok=True); web.run_app(init_app(), port=8000)
