...
<body>
    ...
    <script>
      document.addEventListener('DOMContentLoaded', function() {
          ...
          // --- THIS IS THE FIX ---
          // On page load, check if the server already has an active stream for our session.
          // If it does, switch to the player view immediately to resume it.
          fetch(`/is_active/${sessionId}`)
            .then(r => r.json())
            .then(data => {
                if (data.active) {
                    console.log("Active session found on server. Resuming player view.");
                    showPlayerView(true);
                }
            })
            .catch(e => {
...
async def process_stream_task(session_id, magnet_url, video_index):
        ...
        try:
            def run_ffprobe_json(cmd):
                ...
                if result.returncode != 0:
                    logging.error(f"ffprobe failed for {session_id}: {result.stderr}")
                    return None
            ...
            if video_codec == 'h264' and audio_codec in compatible_audio:
                ...
                command = (f"ffmpeg -i {shlex.quote(stream_api_url)} -map 0 -c copy -f hls -hls_time 4 -hls_playlist_type event -hls_segment_filename 'segment%05d.ts' playlist.m3u8")
            else:
                ...
                command = (f"mpv {shlex.quote(stream_api_url)} --no-terminal --o=- --of=mpegts --oac=aac --ovc=libx264 --ovcopts=preset=ultrafast "
                           f"| ffmpeg -fflags +genpts -i - -map 0 -c copy -f hls -hls_time 4 -hls_playlist_type event -hls_segment_filename 'segment%05d.ts' playlist.m3u8")
        except subprocess.TimeoutExpired:
            update_status("Media analysis timed out. Defaulting to high-compatibility transcoding...")
            command = (f"mpv {shlex.quote(stream_api_url)} --no-terminal --o=- --of=mpegts --oac=aac --ovc=libx264 --ovcopts=preset=ultrafast "
                       f"| ffmpeg -fflags +genpts -i - -map 0 -c copy -f hls -hls_time 4 -hls_playlist_type event -hls_segment_filename 'segment%05d.ts' playlist.m3u8")
        except Exception as e:
            logging.error(f"Error during ffprobe analysis for {session_id}: {e}", exc_info=True)
            update_status("Error analyzing media. Cannot start stream.")
...
async def download_and_convert_subtitle(app, session_id, magnet_url, subtitle_index):
    ...
    try:
        async with app['http_client'].get(subtitle_stream_url, timeout=30) as response:
            ...
            else: logging.warning(f"Failed to download subtitle (HTTP {response.status}) for session {session_id}")
    except Exception as e: logging.error(f"Subtitle download exception for {session_id}: {e}", exc_info=True)
        ...
        try:
            ...
            with streams_lock:
                ...
                for sid, data in list(active_streams.items()):
                    ...
                    if is_running and current_time - last_seen > SESSION_TIMEOUT_SECONDS: sessions_to_kill.append(sid)
...
async def handle_stream_post(request):
    ...
    # Clean up orphaned streams before starting a new one.
    orphaned_sid_to_clean = None
    with streams_lock:
        for sid, sdata in active_streams.items():
            if sdata.get('magnet_url') == video_url and sid != session_id:
    ...
    if orphaned_sid_to_clean:
        # Perform the actual slow cleanup outside of the main lock
    ...
    with streams_lock:
        ...
        active_streams[session_id] = {'process': None, 'last_seen': time.time(), 'process_running': False, 
                                      'progress_data': {}, 'progress_task': None, 'status_message': 'Initializing...', 'magnet_url': video_url}
    
    asyncio.create_task(process_stream_task(session_id, video_url, video_index))
...
# --- NEW: Endpoint to check if a session is still active on the server ---
async def handle_is_active(request):
    session_id = request.match_info.get('session_id'); validate_session_id(session_id)
    is_active = False
    with streams_lock:
        session_data = active_streams.get(session_id)
        if session_data and session_data.get('process_running'):
            is_active = True
    return web.json_response({"active": is_active})
...
def init_app():
    ...
    app.router.add_get('/is_active/{session_id}', handle_is_active) # New Route
