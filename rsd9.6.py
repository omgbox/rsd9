...
<body>
    ...
    <script>
      document.addEventListener('DOMContentLoaded', function() {
          ...
          let hls = null, pollingInterval = null, sessionId = null, heartbeatInterval = null, progressInterval = null, currentMagnet = null;
          ...
          function showFormView() {
              ...
              torrentNameDisplay.style.display = 'none'; torrentNameDisplay.textContent = '';
              ...
              fetch(`/stop/${sessionId}?hard=true`,{method:'POST'}).finally(() => {
                  ...
                  sessionStorage.removeItem(`playbackPosition_${sessionId}_${currentMagnet}`);
                  sessionStorage.removeItem('hlsStreamerSelectedFile');
                  sessionId = getSessionId();
          ...
          function showPlayerView(isMagnet){
              ...
              document.querySelector("#loading-message h2").textContent = "Preparing stream...";
              const torrentNameDisplay = document.getElementById('torrent-name-display');
              const filename = sessionStorage.getItem('hlsStreamerSelectedFile');
              if (filename) {
                  torrentNameDisplay.textContent = filename;
                  torrentNameDisplay.style.display = 'block';
              } else {
          ...
          const startPollingForProgress = () => {
              ...
              const p=document.getElementById('progress-percentage'),s=document.getElementById('progress-speed'),e=document.getElementById('progress-peers'),z=document.getElementById('progress-size');
              progressInterval=setInterval(()=>{fetch(`/progress/${sessionId}`).then(r=>r.json()).then(d=>{if(!d||!d.infoHash)return;p.textContent=`${d.percentageCompleted.toFixed(2)}%`;s.textContent=d.downloadSpeedHuman;e.textContent=d.connectedPeers;const c=(d.bytesCompleted/1e9).toFixed(2),o=(d.totalBytes/1e9).toFixed(2);z.textContent=`${c} GB / ${o} GB`}).catch(e=>{})},2e3)
          };
          ...
          confirmStreamBtn.addEventListener('click', function(e) {
              ...
              const videoLabel = document.querySelector(`label[for="${selectedVideo.id}"]`);
              if (videoLabel) {
                  const filename = videoLabel.textContent.trim().replace(/\s+\([\d\.]+\s+MB\)$/, '');
                  sessionStorage.setItem('hlsStreamerSelectedFile', filename);
              }
...
async def process_stream_task(session_id, magnet_url, video_index, subtitle_index, start_byte=0, start_segment=0, start_time=0):
...
async def handle_stream_post(request):
    ...
    with streams_lock:
        ...
        active_streams[session_id] = {'process': None, 'last_seen': time.time(), 'process_running': False, 
    ...
    asyncio.create_task(process_stream_task(session_id, video_url, video_index, sub_index, start_time=start_time))
...
async def handle_restart_stream(request):
    ...
    await asyncio.to_thread(stop_stream_process, session_id, cleanup_files=False)
    ...
    with streams_lock:
        active_streams[session_id] = {'process': None, 'last_seen': time.time(), 'process_running': False, 
                                      'progress_data': progress_data_copy, 'progress_task': None, 'status_message': 'Restarting...', 
                                      'stream_context': context_copy}

    asyncio.create_task(process_stream_task(session_id, context_copy['magnet_url'], context_copy['video_index'], context_copy['subtitle_index'], start_byte=start_byte, start_segment=next_segment_number, start_time=start_time))
    ...
    progress_task = asyncio.create_task(poll_stream_progress(request.app, session_id, context_copy['magnet_url']))
    with streams_lock:
        if session_id in active_streams: active_streams[session_id]['progress_task'] = progress_task
...
async def handle_is_active(request):
    ...
    with streams_lock:
        ...
        if session_data:
            ...
            if session_data.get('process_running') and session_data.get('process') and session_data['process'].poll() is None:
