...
async def stop_stream_process_async(session_id, cleanup_files=False):
    with streams_lock:
        ...
        if not session_data and cleanup_files:
            ...
            if os.path.isdir(session_dir): await asyncio.to_thread(shutil.rmtree, session_dir, ignore_errors=True)
            return
        ...
        try:
            ...
            with streams_lock:
                for sid, data in list(active_streams.items()):
                    ...
                    if is_running and proc and proc.poll() is not None: data['process_running'] = False; is_running = False
            ...
            tasks = [stop_stream_process_async(sid, False) for sid in sessions_to_kill] + [stop_stream_process_async(sid, True) for sid in sessions_to_delete]
...
async def handle_stream_post(request):
    ...
    # CORRECTED LOGIC: First, asynchronously clean up any existing stream for this session.
    await stop_stream_process_async(session_id, cleanup_files=True)
    
    # THEN, start the new stream process in a separate thread to avoid blocking.
    processed_url = f"{STREAM_API_URL}{quote(url)}&index={v_idx}" if url.startswith("magnet:?") else url
...
async def cleanup_on_shutdown(app):
    ...
    tasks = [stop_stream_process_async(sid, True) for sid in list(active_streams.keys())]
    await asyncio.gather(*tasks)
...
def main():
    ...
    system_status = {"is_ramdisk": os.path.ismount(abs_streams_path), "ramdisk_command": f"sudo mount -t tmpfs -o size={RAMDISK_SIZE} tmpfs {abs_streams_path}"}
