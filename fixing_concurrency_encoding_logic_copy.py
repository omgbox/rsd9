...
# --- 4. Main Application HTML (No changes) ---
APP_HTML = """
<!DOCTYPE html>
<html lang="en">
<head>
...
def stop_stream_process(session_id, cleanup_files=False):
    # --- THIS IS THE FIX ---
    # 1. Pop the session data from the dictionary atomically to get the process handle.
    ...
    # 2. If there was no session, just try to clean up files and exit.
    if not session_data:
        if cleanup_files:
            shutil.rmtree(os.path.join(STREAMS_BASE_DIR, str(session_id)), ignore_errors=True)
        return

    # 3. Perform all slow, blocking operations *outside* the lock.
    #    This prevents the deadlock that was crashing other users' streams.
    if (proc := session_data.get('process')) and proc.poll() is None:
        ...
        proc.kill()  # Use kill() for immediate, forceful termination
        try:
            proc.wait(timeout=5)
    ...
    # 4. Clean up files now that the process is guaranteed to be gone.
    if cleanup_files:
...
async def process_stream_task(session_id, magnet_url, video_index):
    ...
    try:
        ...
        try:
            ...
            ffprobe_cmd = (f"ffprobe -v quiet -print_format json -show_streams "
                           f"-select_streams v:0,a:0 {shlex.quote(stream_api_url)}")
...
async def reaper_task(app):
    while True:
        ...
        try:
            ...
            logging.error("Error in reaper task:", exc_info=True)
...
async def handle_stream_post(request):
    ...
    with streams_lock:
        ...
        active_streams[session_id] = {'process': None, 'last_seen': time.time(), 'process_running': False, 'progress_data': {}, 'progress_task': None, 'status_message': 'Initializing...'}
...
async def cleanup_on_shutdown(app):
    ...
    for sid in sids: stop_stream_process(sid, cleanup_files=True)
